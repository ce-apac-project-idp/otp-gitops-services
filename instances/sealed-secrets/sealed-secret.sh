...
# Create Kubernetes Secret yaml in namespace "external-secrets"
oc create secret generic secrets-manager -n external-secrets \
   --from-literal apiKey=${API_KEY} \
   --dry-run=client -o yaml > delete-api-key-secret.yaml

# Encrypt the secret using kubeseal and private key from the cluster
kubeseal -n external-secrets --controller-name=${SEALED_SECRET_CONTOLLER_NAME} --controller-namespace=${SEALED_SECRET_NAMESPACE} -o yaml < delete-api-key-secret.yaml > sealed-secret.yaml
